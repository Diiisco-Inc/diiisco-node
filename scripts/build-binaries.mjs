...
// ---------------------------------------------------------------------------
//
// Embedded — as an in-memory manifest, not a directory mount.
//
// Bun's standalone filesystem exposes embedded *files* but has no
// **directories**: `existsSync` on a bunfs directory is false and `readdirSync`
// throws ENOENT. So `src/api/statusPages.ts` serves from
// `src/api/webManifest.generated.json`, which the bundler inlines. That file is
// committed and regenerated by `npm run build:web`; if it is stale relative to
// `dist/web`, the binary would ship the previous UI, so say so loudly.
function reportWebAssets() {
  const manifestPath = join(root, 'src', 'api', 'webManifest.generated.json');
  let count = 0;
  try {
    count = Object.keys(JSON.parse(readFileSync(manifestPath, 'utf8')).files ?? {}).length;
  } catch {
    count = 0;
  }

  if (count === 0) {
    console.log('  status pages: manifest is empty — the binary serves the built-in fallback shell.');
    console.log('                run `npm run build:web` to embed them.');
    return;
  }

  console.log(`  status pages: ${count} asset(s) embedded from src/api/webManifest.generated.json`);

  if (existsSync(join(root, 'dist', 'web', 'index.html'))) {
    const check = spawnSync(process.execPath, [join(root, 'scripts', 'build-web-manifest.mjs'), '--check'], {
      cwd: root,
      encoding: 'utf8',
    });
    if (check.status !== 0) {
      console.log('  ⚠ the manifest does not match the current dist/web build — run `npm run build:web`.');
    }
  }
}
...
/**
 * Every committed file under src/, concatenated — the "this is not a secret"
 * corpus.
 *
 * `webManifest.generated.json` is excluded: it is a committed blob of built web
 * assets, so treating it as a source of known-public literals would let a
 * secret that somehow landed in the status-page bundle launder itself past the
 * scan below.
 */
function committedSources() {
  ...
  return listed.stdout
    .split('\n')
    .filter(Boolean)
    .filter((relative) => !relative.endsWith('webManifest.generated.json'))
    .map((relative) => {
      try {
